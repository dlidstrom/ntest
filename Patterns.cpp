...
u2 Base2ToORIDx(u2 pattern2, u2 size) {
   ...
   QSSERT(base3ToORIDTable[size][pattern3]<nORIDs[size]);
...
u2 CMap::IDToConfig(u2 id) const {
   ...
   case kCRID:
      ...
      QSSERT(id<nCRIDs[size]);
      QSSERT(size==6 || size==10);
...
std::vector<u1> configToPotMob[2][9];
std::vector<u1> configToMob[2][9];
std::vector<u1> configToPotMobTriangle[2];
std::vector<u1> configToMobTriangle[2];
   ...
   if (!fInitConfigToPotMob) {
      ...
      for (length=3, nConfigs=27; length<=N; length++, nConfigs*=3) {
         for (color=0; color<2; color++) {
            configToPotMob[color][length].resize(nConfigs);
            configToMob[color][length].resize(nConfigs);
      ...
      configToPotMobTriangle[0].resize(nConfigs);
      configToPotMobTriangle[1].resize(nConfigs);
      //CHECKNEW(configToPotMobTriangle[0] && configToPotMobTriangle[1]);
      configToMobTriangle[0].resize(nConfigs);
      configToMobTriangle[1].resize(nConfigs);
